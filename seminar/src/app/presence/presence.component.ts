...
export class PresenceComponent implements OnInit {
  ...
  // }
  constructor(
    public service: schduleSevice, public serviceClass: ClassSevice,
    public serviceSubject: SubjectSevice,
    public serviceTeacher: teacherService,
    public servicePresence: ServicePresence) { 
      ...
      this.serviceClass.getAllClass().subscribe((res) => {
        ...
        this.serviceTeacher.GetAllTeacher().subscribe((res) => {
          ...
          this.techerListFilter = this.teacherList;
            ...
            for (let index = 0; index < this.Scheduler.length; index++) {
              let element = this.Scheduler[index];
...
{ 
 let s= this.getSchedule(Day,hour,c);
  ...
  getSchedule(Day: number, hour: number, c: Class) {
    ...
    for (let index = 0; index < this.Schedule.length; index++) {
      ...
      if (item.Day == Day
        ...
        && c.CodeClass == item.Clss.CodeClass) {l=item
         // if(d==0){ d=1;l[0]=item}
       // else 
  ...
  addTeacher( Day: number, hour: number, c: Class) {
    let b: boolean;
    b = true;
    var a = this.getSchedule(Day, hour, c);
    
    this.listP.forEach(element => {
      if (a.Techer.CodeTeacher == element.CodeTecher && a.Day == element.Day && a.Hour_in_dey == element.Mis_Hour) {
        b = false;
        return false;
      }
    })
    ...
    if (b == true) {
      this.p = new presenceMat(a.Techer, new Teacher(), false, a.Hour_in_dey, a.Day,a.Clss);
      this.listPresense.push(this.p);
      this.listP.push(new addPresence(this.rangeDates[0], hour, 3, a.Techer.CodeTeacher, Day))
  ...
  let b: boolean;
  b = true;
  var a = this.getScheduleT(Day, hour, c);
...
addTeacherBySubject( Day: number, hour: number, c:subject) 
{
  let b: boolean;
  b = true;
  var a = this.getScheduleSubject(Day, hour, c);
 ...
 addVaction() {
   ...
   for (let index = 0; index < this.rangeDates.length; index++) {
      ...
      for (let index1 = 0; index1 < this.Schedule.length; index1++)
        this.servicePresence.setPresences(new addPresence(
          this.rangeDates[this.Schedule[index1].Day],
          this.Schedule[index1].Hour_in_dey,
          this.s,
          this.Schedule[index1].Techer.CodeTeacher,
  ...
  delet(i:number) {
let l=this.listPresense[i];
    this.listP.forEach(element => {
      if (l.teacher1.CodeTeacher == element.CodeTecher && l.day == element.Day && l.hour == element.Mis_Hour)
      {var i= this.listP.indexOf(element);
          this.listP.splice(i);
        }
      if (this.listPresense[i].teacher2.CodeTeacher == element.CodeTecher)
      {
        var i= this.listP.indexOf(element);
          this.listP.splice(i);
      }
  ...
  addPresence() {
   ...
   console.log(this.rangeDates) 
    ...
    this.listPresense.forEach(element => {
      ...
      for (let index = 0; index < this.Schedule.length; index++) {
     // if (this.listTP.includes(this.Schedule[index].Techer) == false&&this.)
     if(this.ScheduleAddTeacher.includes(this.Schedule[index])==false)
     {
        this.listP.push(new addPresence(
          ...
          Stutus.present,
          this.Schedule[index].Techer.CodeTeacher,
    ...
    this.listP.forEach(p => {
      this.servicePresence.setPresences(p)
    });
  ...
  addDay(l:presenceMat)
  {
   ...
   if(element.Techer.CodeTeacher==l.teacher1.CodeTeacher)
